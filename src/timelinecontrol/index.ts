...
import { IDataGroups, IDataItems, IListItems } from "./type";
...
class TimelineData {
  //an array that stores the timeline data
  id: string;
  group: string;
  content: string;
  //subgroup: number;
  ...
  implements ComponentFramework.StandardControl<IInputs, IOutputs>
{
  ...
  public updateGlobalVars(updates: {
    ...
    if (updates.reloadTimeline !== undefined)
      this._reloadTimeline = updates.reloadTimeline;
    if (updates.reloadData !== undefined) this._reloadData = updates.reloadData;
    if (updates.selectedModifier !== undefined)
      this._selectedModifier = updates.selectedModifier;
    if (updates.stack !== undefined) this._stack = updates.stack;
    if (updates.stackSubgroups !== undefined)
      this._stackSubgroups = updates.stackSubgroups;
    if (updates.verticalScroll !== undefined)
      this._verticalScroll = updates.verticalScroll;
    if (updates.horizontalScroll !== undefined)
      this._horizontalScroll = updates.horizontalScroll;
  ...
  public init(
    ...
    const listItems = JSON.parse(context.parameters.listItems.raw || "[]") as IListItems;
    ...
    this.showListItems(context.parameters.editMode.raw,listItemsDiv);
  ...
  public updateView(context: ComponentFramework.Context<IInputs>): void {
    ...
    if (this._listItems !== context.parameters.listItems.raw) {
      const listItems = JSON.parse(context.parameters.listItems.raw || "[]") as IListItems;
      console.log("adhasd", listItems);
      this.updateListItems(listItems, context);
      this.updateGlobalVars({
        listItems: context.parameters.listItems.raw || undefined,
      });
    }
    this.showListItems(context.parameters.editMode.raw);
    ...
    if (!this._timelineVis) {
      ...
      this._timelineVis = new Timeline(
        this._timelineElm,
        this._timelineData,
   ...
   // Update the timeline's data
    if (
      this._stack !== context.parameters.optionStack.raw ||
      ...
      this._horizontalScroll !==
        context.parameters.optionHorizontalScroll.raw ||
      this._editMode !== context.parameters.editMode.raw ||
      String(this._start) !== context.parameters.optionStart.raw?.toISOString() ||
      String(this._end) !==context.parameters.optionEnd.raw?.toISOString()
    ) {
    ...
    if (
      this._dataitems !== context.parameters.dataitems.raw ||
      this._reloadData !== context.parameters.reloadData.raw
    ) {
      console.log(
        "dataitems: " + (this._dataitems !== context.parameters.dataitems.raw)
      );
      console.log(
        "reloadData: " +
          (this._reloadData !== context.parameters.reloadData.raw)
  ...
  private injectCustomCss(context: ComponentFramework.Context<IInputs>): void {
    let customCss =
      (context.parameters.customCss.raw || "") +
      (context.parameters.listCSS.raw || "");
  ...
  private getTimelineOptions(
    ...
    let options: TimelineOptions = {
      ...
      stack:
        context.parameters.optionStack.raw !== null
          ? context.parameters.optionStack.raw
          : false,
      stackSubgroups:
        context.parameters.optionStackSubgroups.raw !== null
          ? context.parameters.optionStackSubgroups.raw
          : false,
      verticalScroll:
        context.parameters.optionVerticalScroll.raw !== null
          ? context.parameters.optionVerticalScroll.raw
          : true,
      horizontalScroll:
        context.parameters.optionHorizontalScroll.raw !== null
          ? context.parameters.optionHorizontalScroll.raw
          : true,
      autoResize: true,
      minHeight: 100,
  ...
  private createTimelineData(
    ...
    let parsedItems = JSON.parse(dataitems) as IDataItems;
  ...
  private updateListItems(
    ...
    const listItemsDiv = div
      ? div
      : (document.getElementById("list-items-container") as HTMLDivElement);
    // Clear existing list items
    while (listItemsDiv?.firstChild) {
      listItemsDiv.removeChild(listItemsDiv.firstChild);
    }
    
      console.log("erdhi", listItems, listItemsDiv);
      ...
      listItems.forEach((item) => {
        ...
        listItem.addEventListener(
          "dragstart",
          (event: DragEvent) => this.handleDragStart(event, item),
          false
        );
        listItem.addEventListener(
          "dragend",
          (event: DragEvent) => this.handleDragEnd(event, item, context),
          false
        );
  ...
  private showListItems(editMode: boolean, div?: HTMLDivElement) {
    const listItemsDiv = div
      ? div
      : (document.getElementById("list-items-container") as HTMLDivElement);
    if (editMode) {
      if (listItemsDiv) listItemsDiv.style.display = "block";
    } else {
      if (listItemsDiv) listItemsDiv.style.display = "none";
    }
  }
  private createTimelineGroups(
    context: ComponentFramework.Context<IInputs>
  ): void {
    console.warn("sdfsd");
    let datagroups = context.parameters.datagroups.raw || "[]";
    let parsedItems = JSON.parse(datagroups) as IDataGroups;
    this._timelineGroups = [];
